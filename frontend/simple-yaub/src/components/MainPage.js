...
import {fetchPostsMetaInfo} from "./Backend";
import {NavigationBar} from "./NavBar";

const PostsMetaInfo = fetchPostsMetaInfo();
        ...
        posts.forEach(post => {
            if (post.tags && Array.isArray(post.tags)) {
                post.tags.forEach(tag => {
                    uniqueTags.add(tag); // Add the tag to the Set
                });
            }
        });
...
const MainPage = () => {
    ...
    const filteredPosts = selectedTag === 'All'
        ? PostsMetaInfo.posts
        : PostsMetaInfo.posts.filter((post) => post.tags.includes(selectedTag));

    const availableTags = getUniqueTags(PostsMetaInfo.posts);
                ...
                {filteredPosts.map((post) => (
                    <Col md={4} key={post.id} className="mb-4">
                        ...
                        <Card.Body>
                                ...
                                <Link to={`/post/${post.id}`} className="custom-btn custom-btn-main">
                                    Read More
                                </Link>
                            </Card.Body>
                        </Card>
